      ...
      <div className="bg-green-900 px-4 pt-4 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h2 className="text-white font-black text-lg leading-tight">{player.name}</h2>
            {standing && (
              <span className="text-clay-400 text-sm font-bold flex items-center gap-1 mt-0.5">
                <Crown size={12} /> Ranking {standing.position}
              </span>
            )}
          </div>
          {player.group_letter && (
      ...
      <div className="px-4 -mt-3 space-y-3 pb-4">
        ...
        {nextMatch && (
          <Card className="border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Próximo Jogo</span>
            ...
            {nextMatch.group_letter && (
              <p className="text-center text-[11px] text-gray-400 mt-2">
                Grupo {nextMatch.group_letter} • {nextMatch.court ?? "Quadra a definir"}
              </p>
            )}
          </Card>
        )}
        ...
        <Card className="border border-gray-100">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              { icon: <CheckCircle2 size={18} className="text-green-700" />, label: "Vitória", value: wins, color: "text-green-700" },
              { icon: <Clock size={18} className="text-amber-500" />, label: "Pendente", value: pending, color: "text-amber-500" },
              { icon: <XCircle size={18} className="text-clay-500" />, label: "Derrota", value: losses, color: "text-clay-500" },
              { icon: <Minus size={18} className="text-gray-400" />, label: "WO", value: wo, color: "text-gray-400" },
          ...
          <div className="grid grid-cols-3 gap-3">
            <Link href="/calendario">
              <div className="bg-green-900 rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition-transform">
                ...
                <span className="text-green-100 text-[11px] font-bold text-center leading-tight">Informar Disponibilidade</span>
              </div>
            </Link>
            <Link href="/partidas/novo">
              <div className="bg-clay-500 rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition-transform">
                ...
                <span className="text-white text-[11px] font-bold text-center leading-tight">Registrar Resultado</span>
              </div>
            </Link>
            <Link href="/ranking">
              <div className="bg-white border border-gray-100 rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition-transform shadow-card">
                ...
                <span className="text-green-900 text-[11px] font-bold text-center leading-tight">Ver Ranking</span>
              </div>
            </Link>
          </div>
        </div>
        ...
        <Card className="border border-gray-100" padding="none">
          <div className="flex items-center justify-between px-4 pt-4 pb-3">
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Histórico</span>
            <Link href="/calendario" className="text-clay-500 text-xs font-semibold flex items-center gap-0.5">
              Ver todas <ChevronRight size={12} />
            </Link>
          </div>
          <div className="divide-y divide-gray-50">
            {recentMatches.length === 0 && (
              <p className="text-center text-gray-400 text-sm py-6 px-4">Nenhuma partida jogada ainda</p>
            )}
              ...
              return (
                <div key={match.id} className="flex items-center px-4 py-3 gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">vs {opponent?.name}</p>
                    <p className="text-xs text-gray-400">
                      {match.scheduled_at ? format(new Date(match.scheduled_at), "d MMM", { locale: ptBR }) : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-black ${result === "victory" ? "text-green-700" : result === "defeat" ? "text-clay-500" : "text-gray-400"}`}>
                      {score}
                    </span>
          ...
          <Card className="border border-gray-100">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-4xl font-black text-green-900">{standing.position}º</span>
                <span className="text-[10px] text-gray-400 font-medium">Posição</span>
              </div>
