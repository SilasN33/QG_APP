...
import type { GroupLetter, Standing } from "@/types";
    ...
    <div className="overflow-x-auto">
          ...
          {standings.map((s, i) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={cn(
                        ...
                        isClassified ? "text-gray-900" : "text-gray-600"
                      )}
                    >
                      {s.player.name}
                    </span>
                ...
                <td className="py-2.5 text-center font-black text-gray-800 text-xs">
                  {s.points}
                </td>
      ...
      <div className="flex items-center gap-4 mt-3 pt-2 border-t border-gray-100">
        ...
        <span className="text-[10px] text-gray-300 ml-auto">
          P: Pontos · V: Vitórias · D: Derrotas
        </span>
      </div>
    </div>
  );
...
function MatchCard({
  ...
  return (
    <div className="flex items-center py-3 px-4 gap-2 border-b border-gray-50 last:border-0">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {match.court && (
            <span className="text-[10px] text-gray-300">· {match.court}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="text-center min-w-[48px]">
            {match.status === "wo" ? (
              ...
              <span className="text-xs font-black text-gray-700">
                {match.sets
                  .filter((s) => s.player1_games > s.player2_games)
                  .length
                  }-
                {match.sets
                  .filter((s) => s.player2_games > s.player1_games)
                  .length}
              </span>
            ) : (
        ...
        {hasResult && match.sets.length > 0 && (
          <div className="flex justify-center gap-1 mt-1">
            {match.sets.map((s) => (
              <span key={s.set_number} className="text-[10px] text-gray-400">
...
export default function GruposPage() {
  ...
  const groupStandings = MOCK_STANDINGS.filter(
    (s) => s.group_letter === activeGroup
  ).sort((a, b) => a.position - b.position);

  const groupMatches = MOCK_MATCHES.filter(
    (m) => m.group_letter === activeGroup
  ).sort((a, b) => {
