-- ============================================================
-- QG Open 2026 — RESET & NEW SCHEMA
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1. Drop everything from the MVP
DROP TABLE IF EXISTS match_sets CASCADE;
DROP TABLE IF EXISTS matches    CASCADE;
DROP TABLE IF EXISTS players    CASCADE;

-- 2. Players
--    group_letter is now nullable (admin assigns later)
--    is_admin flag to control access to admin panel
CREATE TABLE players (
  id           uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  name         text        NOT NULL,
  avatar_url   text,
  group_letter text        CHECK (group_letter IN ('A','B','C','D')),  -- nullable
...
-- Players can insert their own profile (created on signup)
CREATE POLICY "Users can insert own player" ON players
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Players can update their own profile (name, avatar)
CREATE POLICY "Users can update own player" ON players
  FOR UPDATE USING (auth.uid() = user_id);

-- Admins can do everything on players
...
-- Admins can manage sets
CREATE POLICY "Admins full access match_sets" ON match_sets
  FOR ALL USING (
    EXISTS (SELECT 1 FROM players WHERE user_id = auth.uid() AND is_admin = true)
  );

-- Players can submit results for their own matches
...
-- ============================================================
-- UPDATE players SET is_admin = true WHERE user_id = auth.uid();
-- OR by email:
